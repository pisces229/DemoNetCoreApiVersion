...
namespace Test.MediaType
{
    [TestClass]
    public class TestMediaType : TestInitialize
    {
        private const string Host = "api/MediaType";
        [TestMethod("Version")]
        ...
        {
            var requestUri = $"{Host}";
            _httpClient.DefaultRequestHeaders.Add("Accept", "application/json;version=3.0");
            var response = await _httpClient.GetAsync(requestUri);
            Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
            await DisplayHttpResponseMessage(response);
        }
